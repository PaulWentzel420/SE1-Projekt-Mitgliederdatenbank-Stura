...
Es sollen verschiedene Nutzergruppen angelegt werden, welche über unterschiedliche Rechte verfügen.
...
* Der Server auf dem die Webseite laufen soll, bietet ausreichende Ressourcen, welches noch geklärt werden müssen
...
* Wir sind von den gegebenen Ressourcen des StuRas abhängig. Diese belaufen sich auf einen Docker Container des hauseigenen Servers.
...
. Bei der Wahl der Programmiersprache haben wir uns für Python entschieden, da uns der StuRa die Vorgabe gab kein Java zu verwenden. Des weiteren hat Python eine gut verständliche Syntax und ist vielfältig einsetzbar.
. Wir entschieden uns für das Django Webframework, da dieses eine größe Community und somit großen Support bietet. Zusätzlich ist es ein Open-Source-Projekt.
. Wir haben uns für ein Webdesign entschieden, welches der HTW-Website ähnelt, um eine vertraute Nutzerumgebung zu schaffen.
. Für das Datenbanksystem werden wir SQLite verwenden, unter Anderem da unsere Vorgänger dies bereits nutzten. Außerdem ist es Open-Source und wird standardmäßig von Django unterstützt.
