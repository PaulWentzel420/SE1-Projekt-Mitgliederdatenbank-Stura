...
** Zur Validierung können mit verschiedenen Test-Nutzern Daten verändert, gelöscht und hinzugefügt werden. Anschließend werden alle alle getätigten Änderungen mit der Historie abgeglichen.
...
Website sowie Datenbank benötigen keine 99,99% Erreichbarkeit, sollten jedoch so oft wie möglich verfügbar sein. Die Benutzereingaben sind auf Sinnhaftigkeit zu überprüfen um Falscheingaben zu vermeiden (z.B. keine Postleitzahl im Namensfeld). Fehlende Pflichtdaten führen zu einer Eingabeaufforderung an den Nutzer und lösen noch keine Änderung an der Datenbank aus. Änderungen sollen dokumentiert werden (z.B. Nutzer A löscht Mitglied B am 01.01.2023). Systemausfälle sollen mittels Log-Dateien nachvollziehbar sein.

=== Effizienz (Performance)
Auf die Website sollen bis zu 3 Nutzer gleichzeitig zugreifen können. Datenbankabfragen sollten auch bei hoher Mitgliederanzahl innerhalb von 5 Sekunden ausgeführt sein. Das Anlegen neuer Datensätze soll direkt erfolgen, um reibungslose Arbeitsabläufe zu gewährleisten.
...
Es soll eine Installationsanleitung beigelegt werden, welche das selbständige (neu-)installieren der Software durch die Nutzer ermöglicht. Berechtigte Nutzer sollen Tabellen selbständig verwalten (z.B. Anlegen neuer Spalten) können.
...
Es soll kein Java verwendet werden. Das System soll auf einem beliebigen Linux-Betriebssystem lauffähig sein.
