...
| einfache Verwaltung der Mitglieder | hoch | Eigener Button für "Mitglieder", in dem ein bestehendes Mitglied bearbeitet werden kann. Hier sollen auch neue Mitglieder angelegt bzw. gelöscht werden können. | xx
| Ausgabe der Mitwirkungsdauer von Mitgliedern | mittel | Es soll ein eigener Button "Mitwirkungsdauer" entstehen. Hier können von allen oder nur bestimmten Mitgliedern die Mitwirkungsdauer eingesehen werden. | xx
| Ausgabe der Bearbeitungshistorie | niedrig | Eigener Button der es Admins ermöglicht, die Beabreitungshistorie anzeigen zu lassen. | xx
| Daten im- und exportieren | niedrig | Es muss möglich sein, vorhandene Daten in die Mitgliederdatenbank aufzunehmen und auch wieder in bestimmten Formaten auszugeben | xx
| Ausgabe von Stimmberechtigungen | mittel | Es sollte möglich sein, dass über einen eigenen Button alle stimmberechtigten StuRa-Mitglieder in einer Liste aufgelistet werden | xx
|===
